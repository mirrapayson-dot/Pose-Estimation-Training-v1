...
CAT_PERSON     = "person"
CAT_POLE       = "poles"
CAT_ROLLERSKI  = "rollerskis"

NUM_KP_PERSON     = 15
NUM_KP_POLE       = 4
NUM_KP_ROLLERSKI  = 4
TOTAL_KP          = NUM_KP_PERSON + NUM_KP_POLE + NUM_KP_ROLLERSKI
...
def convert():
    ...
    for idx, img_id in enumerate(image_ids):
        ...
        split    = "val" if Path(fname).name in VAL_FRAMES else "train"
        ...
        for person in person_anns:
            ...
            all_kps = person_kps + pole_kps + ski_kps
            norm_kps = []
            for k in range(TOTAL_KP):
                base = k * 3
                kx  = min(max(all_kps[base]     / W, 0.0), 1.0)
                ky  = min(max(all_kps[base + 1] / H, 0.0), 1.0)
                vis = all_kps[base + 2]
